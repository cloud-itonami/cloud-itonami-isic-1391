(ns knittingops.registry-test
  (:require [clojure.test :refer [deftest is]]
            [knittingops.registry :as r]))
...
(deftest small-shipment-within-volume-does-not-exceed
  (is (false? (r/shipment-volume-exceeded?
               {:volume-yards 20000.0 :shipped-volume-yards 5000.0} 5000.0))))

(deftest shipment-that-pushes-past-volume-exceeds
  (is (true? (r/shipment-volume-exceeded?
              {:volume-yards 6000.0 :shipped-volume-yards 5700.0} 1000.0))))

(deftest shipment-exactly-at-volume-does-not-exceed
  (is (false? (r/shipment-volume-exceeded?
               {:volume-yards 6000.0 :shipped-volume-yards 5700.0} 300.0))
...
(deftest missing-volume-is-not-flagged-exceeded
  ...
  (is (false? (r/shipment-volume-exceeded? {:volume-yards 800.0} nil))))

;; ----------------------------- grade-valid? -----------------------------

(deftest known-grades-are-valid
  (doseq [g [:grade-a :grade-b :grade-c :irregular :reject]]
    (is (r/grade-valid? g))))
