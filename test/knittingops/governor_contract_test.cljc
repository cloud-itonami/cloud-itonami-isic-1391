(ns knittingops.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control circular/flat-knitting or
  crocheting-line equipment directly... does NOT authorize or execute
  equipment operations') implemented faithfully. The single invariant
  under test:

    KnittingAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Knitting Mill Operations
...
(def coordinator {:actor-id "coord-1" :actor-role :mill-coordinator :phase 3})
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:quality-grade :grade-a}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :grade-a (:quality-grade (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-knitting-machine :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
...
(deftest shipment-volume-exceeded-is-held-and-unoverridable
  (testing "a shipment proposal whose yardage would exceed the batch's own logged volume -> HOLD"
    (let [[db actor] (fresh)
