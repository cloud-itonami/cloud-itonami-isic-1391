# Business Model: Manufacture of knitted and crocheted fabrics
...
## Classification
- Repository: `cloud-itonami-isic-1391` ([github.com/cloud-itonami/cloud-itonami-isic-1391](https://github.com/cloud-itonami/cloud-itonami-isic-1391))
- ISIC Rev.5: `1391` — Manufacture of knitted and crocheted fabrics
- Domain: `:knitting/manufacturing`
- Social impact: :worker-safety :labor-standards :materials-traceability
- Actor: `:knitting-mill-operations-governor` — an independent Governor in the fleet's Sealed-LLM
  ⊣ Governor pattern (langgraph-clj StateGraph, append-only audit ledger,
  Phase 0→3 rollout). Robotics authority: full (governed field-equipment authority, HARD-gated, human sign-off on every actuation).

## Customer
An operator running this vertical as an OSS business — :knitting/manufacturing — who wants
a governed execution scaffold they own instead of renting a closed SaaS.

## Offer
The actor coordinates the :spec :design :produce :inspect :package :audit pipeline behind an independent Governor: the
...
Capability stack (required): :robotics :identity :forms :dmn :bpmn :audit-ledger :cae.
...
One managed tenant (:knitting/manufacturing):
...
- Do not bypass the `:knitting-mill-operations-governor` for production decisions.
