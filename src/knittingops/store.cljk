(ns knittingops.store
  "SSoT for the knitting/crocheting-mill plant-operations coordination
  ...
  for dev/tests/demo, no deps. This vertical is self-contained (no
  external knitting capability library, no jurisdiction-scoped Datomic
  parity requirement driving a second backend); a `langchain.db`-backed
  store can be added later behind the same protocol without changing
  any caller.

  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             circular/flat-knitting or crocheting
                             output: quality-grade/yardage/fabric-
                             weight/defect-rate record. `:verified?`
                             marks whether the batch's own claims have
                             actually been QC-inspected (never
                             inferred from a routine intake patch);
                             `:registered?` marks whether it is on
                             file in the mill's production ledger;
                             `:shipped-volume-yards` tracks the
                             batch's own cumulative-shipped ground
                             truth.
    - `equipment`         -- a circular-knitting/flat-knitting/
                             crocheting-line unit's own record.
                             ...
                             `batches`.
    - `maintenance`       -- a scheduled circular/flat-knitting-
                             machine or crocheting-line maintenance-
                             window DRAFT against a piece of equipment
                             (`knittingops.registry`'s
                             `register-maintenance`). Dedicated
                             `:scheduled?` double-schedule guard (never
                             a `:status` value).
    - `shipments`         -- a proposed outbound-fabric-shipment
                             DRAFT (`knittingops.registry`'s
                             `register-shipment`).
  ...
  independently-recomputed yardage, approved by whom, which safety
  concern was flagged' is always a query over an immutable log -- the
  audit trail a mill owner or fabric buyer trusting this coordinator
  needs."
  (:require [knittingops.registry :as registry]))
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (knittingops.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (knittingops.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :quality-grade :grade-a :fabric "cotton-jersey-interlock"
                :volume-yards 20000.0 :fabric-weight-gsm 180.0 :defect-rate-percent 1.5
                :verified? true :registered? true
                :shipped-volume-yards 5000.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :quality-grade :grade-b :fabric "acrylic-crochet-lace"
                :volume-yards 6000.0 :fabric-weight-gsm 220.0 :defect-rate-percent 4.0
                :verified? true :registered? true
                :shipped-volume-yards 5700.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :quality-grade :irregular :fabric "wool-blend-rib-knit"
                :volume-yards 15000.0 :fabric-weight-gsm 300.0 :defect-rate-percent 15.0
                :verified? false :registered? false
                :shipped-volume-yards 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"equip-001" {:id "equip-001" :kind :circular-knitting-machine
                :verified? true :registered? true
                :last-maintenance-date "2026-05-01"}
   "equip-002" {:id "equip-002" :kind :crocheting-line
                :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `knittingops.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `knittingops.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defrecord MemStore [a]
  ...
  (commit-record! [s {:keys [effect path value] :as record}]
    (cond
      ...
      (let [shipment-id (first path)
        ...
        (swap! a (fn [state]
                   (-> state
                       ...
                       (update-in [:batches batch-id :shipped-volume-yards]
                                  (fn [prev]
                                    (+ (double (or prev 0.0))
                                       (double (or (:volume-yards value) 0.0))))))))
        result)
      ...
      ;; above (also what `footwearops`-style siblings expose as their
...
(defn sample-data!
  ...
  shipped (a small new shipment blows through its own logged yardage --
  HARD hold), one UNVERIFIED/unregistered batch (blocks any shipment
  coordinated against it); one verified+registered circular-knitting-
  machine unit (schedulable for maintenance), one UNVERIFIED/
  unregistered crocheting-line unit (blocks any maintenance scheduling
