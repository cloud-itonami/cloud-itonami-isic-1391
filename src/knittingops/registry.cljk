(ns knittingops.registry
  "Pure-function domain logic for the knitting/crocheting-mill plant-
  operations coordination actor -- equipment/batch verification,
  shipment-yardage recompute, quality-grade validation, defect-rate
  ...
  re-verified INDEPENDENTLY by `knittingops.governor` -- the same
  'ground truth, not self-report' discipline every sibling actor's own
  registry establishes (most directly `footwearops.registry`'s
  `shipment-volume-exceeded?`, this build's own closest domain
  analog): never trust a proposal's own self-reported yardage/status
  when the inputs needed to recompute it independently are already on
  record.

  This namespace is pure data + pure functions -- no I/O, no network
  call to any real mill-management system. It builds the DRAFT record
  a mill coordinator would keep (a scheduled maintenance window, a
  coordinated shipment), not the act of actuating a circular/flat-
  knitting machine or crocheting line, or dispatching a real freight
...
(defn equipment-registered?
  ...
  `:registered?` true flag (i.e. it is on file in the mill's
  ...
  actor's HARD invariant ('mill/batch record must be independently
...
(defn batch-verified?
  ...
  (i.e. its quality-grade/yardage/defect-rate claims have actually
...
(defn batch-registered?
  "Ground-truth check: is `batch`'s own record on file in the mill's
...
(defn shipment-volume-exceeded?
  ...
  would `shipped-to-date-yards` + `new-volume-yards` exceed `batch`'s
  own recorded `:volume-yards` (the batch's own logged production
  yardage)? Needs no proposal inspection or stored-verdict lookup --
  ...
  uses (most directly `footwearops.registry/shipment-volume-exceeded?`,
  this build's own closest domain analog)."
  [batch new-volume-yards]
  (let [capacity (:volume-yards batch)
        so-far (:shipped-volume-yards batch 0.0)]
    (and (number? capacity)
         (number? new-volume-yards)
         (number? so-far)
         ...
         (> (Math/round (* 10000 (+ (double so-far) (double new-volume-yards))))
            (Math/round (* 10000 (double capacity))))))) 

(defn shipment-volume-exceeded-checkable?
  "Can `batch`'s headroom actually be computed for `new-volume-yards`?
  ...
  (boolean (and (map? batch)
                (number? (:volume-yards batch))
                (number? (:shipped-volume-yards batch 0.0))
                (number? new-volume-yards))))
...
(defn- unsigned-certificate
  ...
  the human mill supervisor's/shipping approver's act, not this
  actor's."
...
(defn register-maintenance
  ...
  circular/flat-knitting-machine or crocheting-line maintenance window
  against a verified, registered piece of equipment. Pure function --
  does not actuate knitting/crocheting-line equipment or execute any
  maintenance; it builds the RECORD a mill coordinator would keep.
  `knittingops.governor` independently re-verifies the equipment's own
  verified/registered ground truth, and permanently blocks any attempt
  to set `:direct-operate? true` on a maintenance proposal (see README
  `What this actor does NOT do`), before this is ever allowed to
  commit."
...
(defn register-shipment
  ...
  outbound fabric shipment against a verified, registered production
  batch. Pure function -- does not dispatch any real freight carrier;
  it builds the RECORD a mill coordinator would keep.
  `knittingops.governor` independently re-verifies the shipment's own
  claimed yardage against `shipment-volume-exceeded?`, before this is
