(ns knittingops.phase
  "Phase 0->3 staged rollout for the knitting/crocheting-mill plant-
  ...
  consequence (a circular/flat-knitting or crocheting-line maintenance
  window means production downtime, and the equipment ends up actually
  touched); it is always a human mill supervisor's call.
  `knittingops.governor`'s `line-operate-blocked-violations` HARD-blocks
  direct-operate attempts unconditionally, and the confidence/high-
  stakes gate independently never lets `:flag-safety-concern` auto-
  commit either -- multiple independent layers agree on where this
  actor's authority ends. Like every prior sibling's phase-3 `:auto`
  set, this domain has only ONE member (`:log-production-batch`) -- no
...
(defn verdict->disposition
  "Map a Knitting Mill Operations Governor verdict to a base
