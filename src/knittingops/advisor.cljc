(ns knittingops.advisor
  "KnittingAdvisor -- the *contained intelligence node* for the
  knitting/crocheting-mill plant-operations coordination actor.

  It normalizes production-batch patches (quality-grade/yardage/
  fabric-weight/defect-rate), drafts a circular/flat-knitting-machine
  or crocheting-line maintenance scheduling proposal against a piece
  of equipment, drafts a materials-safety/equipment-safety concern
  flag, and drafts an outbound-fabric-shipment coordination proposal
  ...
  cited), never a committed record and NEVER a real knitting/
  crocheting-line-equipment actuation or freight dispatch. Every
  output is censored downstream by `knittingops.governor` before
  anything touches the SSoT -- see README `What this actor does NOT
  do`.
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `knittingops.governor`)
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct knitting/
                                 ; crocheting-line-equipment-control
                                 ; effect
  ...
  `knittingops.governor` HARD-holds any request that doesn't, so a
  mis-wired caller can never reach a commit path even if this advisor
  were compromised."
  (:require #?(:clj  [clojure.edn :as edn]
               :cljs [cljs.reader :as edn])
            [clojure.string :as str]
            [knittingops.registry :as registry]
...
(defn- log-production-batch
  ...
  yardage, fabric-weight or defect-rate. High confidence, low
...
(defn- schedule-maintenance
  "Draft a circular/flat-knitting-machine or crocheting-line
  maintenance-window scheduling proposal against a piece of equipment.
  The advisor reports what it can see (equipment verified?/
  registered?) in its rationale, but `knittingops.governor` NEVER
  trusts this report -- it independently re-derives verified?/
  registered? from the equipment's own stored fields before any
  commit is possible."
...
(defn- flag-safety-concern
  "Draft a materials-safety/equipment-safety concern flag. ALWAYS
  ...
  `knittingops.phase`: no phase ever adds this op to a phase's `:auto`
  set; `knittingops.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound-fabric-shipment coordination proposal against a
  production batch. The advisor passes through the caller's own
  claimed yardage -- it does NOT invent one, and `knittingops.governor`
  NEVER trusts it: it independently recomputes whether the batch's own
  cumulative-shipped yardage plus this claim would exceed the batch's
  own recorded yardage before any commit is possible."
  [db {:keys [subject value]}]
...
(def ^:private system-prompt
  (str "あなたは編み立て・かぎ針編み工場プラント運用コーディネーターの助言者です。"
       ...
       "丸編機・横編機・かぎ針編みラインの設備の直接操作を絶対に提案してはいけません"
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `knittingops.governor`
