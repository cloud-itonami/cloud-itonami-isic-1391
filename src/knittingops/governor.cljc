(ns knittingops.governor
  "Knitting Mill Operations Governor -- the independent compliance
  layer that earns the KnittingAdvisor the right to commit. The
  ...
  schedule) a circular/flat-knitting machine or crocheting line,
  whether a shipment proposal's own claimed yardage would blow through
  the batch's own logged production yardage, or when an act stops
  being a coordination proposal and becomes direct knitting/
  crocheting-line-equipment control, so this MUST be a separate system
  able to *reject* a proposal and fall back to HOLD.

  `:itonami.blueprint/governor` is `:knitting-mill-operations-governor`
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       hallucinated `:knitter/actuate`
                                       or `:crocheter/run`) is the
                                       'direct knitting/crocheting-line-
    ...
    4. Line-operate blocked        -- for `:schedule-maintenance`, does
                                       ...
                                       Directly operating a circular/
                                       flat-knitting machine or
                                       crocheting line (rather than
                                       merely scheduling a DRAFT
                                       maintenance window) is this
    ...
    7. Batch not verified/
       registered                  -- for `:coordinate-shipment`,
                                       ...
                                       `:verified?` AND `:registered?`
                                       are both true
                                       (`knittingops.registry/batch-
                                       ready?`) -- never trust the
                                       advisor's own rationale. Also
                                       part of the 'mill/batch record'
   ...
   11. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       mill supervisor. SOFT: the human
                                       may approve."
...
(def allowed-proposal-effects
  ...
  -- all four are propose-shaped drafts, NEVER a direct knitting/
  crocheting-line-equipment-control effect."
  #{:batch/upsert :maintenance/schedule
    :safety-concern/flag :shipment/propose})

(def high-stakes
  "Stakes grave enough to always require a human, even when clean.
  Safety concerns (materials-safety/equipment-safety) are the one op
  in this domain that always demands human eyes regardless of
  confidence."
  #{:coordination/safety-concern})
...
(defn- equipment-control-blocked-violations
  ...
  Anything else (direct circular/flat-knitting-machine or crocheting-
  line-equipment control, a fabricated actuation effect) is this
  ...
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :equipment-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") は丸編機・横編機・かぎ針編みライン等の設備の直接操作に該当する可能性があり、恒久的に禁止")}]))
...
(defn- line-operate-blocked-violations
  ...
  directly operate a circular/flat-knitting machine or crocheting line
  ...
  (when (and (= op :schedule-maintenance)
    ...
    [{:rule :line-operate-blocked
      :detail "丸編機・横編機・かぎ針編みライン設備の直接操作提案は恒久的に禁止 -- 保守作業予定(draft)のみ許可"}]))
...
(defn- equipment-not-verified-violations
  ...
  ('mill/batch record must be independently verified/registered
  before any action')."
  [{:keys [op]} proposal st]
...
(defn- batch-not-verified-violations
  ...
  trust the advisor's own report. Also part of the 'mill/batch record
    ...
    (let [{:keys [batch-id volume-yards]} (:value proposal)
      ...
      (when (and b (registry/shipment-volume-exceeded? b volume-yards))
        [{:rule :shipment-volume-exceeded
          :detail (str batch-id " の記録済み生産量(" (:volume-yards b)
...
(defn check
  "Censors a KnittingAdvisor proposal against the governor rules.
