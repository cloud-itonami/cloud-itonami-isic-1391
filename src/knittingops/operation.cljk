(ns knittingops.operation
  "KnittingOperationActor -- one plant-operations coordination request =
  one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (KnittingAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Knitting Mill
  ...
  each operation is auditable and checkpointed. A mill's lifecycle is
  ...
  decision to a human mill supervisor/shipping approver. The approver
  resumes with `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [knittingops.advisor :as advisor]
            [knittingops.governor :as governor]
            [knittingops.phase :as phase]
...
(defn build
  "Compiles a KnittingOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `knittingops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; KnittingAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Knitting Mill Operations Governor -- independent censor.
      (g/add-node :govern
