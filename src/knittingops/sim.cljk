(ns knittingops.sim
  ...
  production yardage, a proposal that tries to directly operate a
  circular/flat-knitting machine or crocheting line (permanently
  blocked, no override), a double-schedule of the same maintenance
  window, a production-batch patch with a fabricated grade, and a
...
(defn -main [& _args]
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println "== schedule-maintenance mnt-1 on equip-001 (verified, registered, circular-knitting-machine -- escalates, approve) ==")
    (let [r (exec-op actor "t2"
                      ...
                      coordinator)]
      (println r)
      (println "-- human mill supervisor approves --")
    ...
    (println (exec-op actor "t6"
                       {:op :actuate-knitting-machine :effect :propose :subject "batch-001"}
                       coordinator))

    (println "== schedule-maintenance mnt-2 on equip-002 (UNVERIFIED/unregistered crocheting line -> HARD hold) ==")
